...
-- Составной Для JOIN performance с другими таблицами
--CREATE INDEX idx_performance_covering 
--ON performance(id_author, code_song);

-- Покрывающий Для subscription
--CREATE INDEX idx_subscription_artist 
--ON subscription(id_author_artist) INCLUDE (id_author_user);

-----------------

-- CREATE INDEX idx_listening_optimal 
-- ON listeningHistory(code_song) 
-- INCLUDE (during, id_author);

CREATE INDEX idx_listening_optimal 
ON listeningHistory(code_song, code_listening) 
INCLUDE (id_author);

-- Покрывающий Для подзапроса comment (AVG с GROUP BY)
CREATE INDEX idx_comment_code_song 
ON comment(code_song) INCLUDE (grade);


DROP INDEX IF EXISTS
	idx_performance_covering,
	idx_comment_code_song,
	idx_subscription_artist,
